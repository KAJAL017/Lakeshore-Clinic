...
<div class="space-y-6">
    <x-page-header title="Telemedicine Appointments">
        <x-slot name="subtitle">Your virtual consultations</x-slot>
    </x-page-header>
            ...
            <table class="w-full">
                <thead>
                    <tr class="border-b border-surface-border">
                        ...
                        <th class="px-4 py-3 text-left text-xs font-semibold text-text-muted uppercase">Reason</th>
                ...
                <tbody class="divide-y divide-surface-border">
                    @forelse($appointments as $appointment)
                        <tr class="hover:bg-surface transition-colors">
                            ...
                            <td class="px-4 py-3 text-sm text-text-muted max-w-xs truncate">{{ $appointment->reason ?? '-' }}</td>
                            <td class="px-4 py-3">
                                @php
                                    $statusVariant = match($appointment->status) {
                                        'pending' => 'warning', 'approved' => 'success', 'scheduled' => 'info',
