...
@section('title', 'Doctor Availability - Lakeshore Clinic')
@section('page-title', 'Doctor Availability')

@section('content')
<div class="space-y-6">
    <x-page-header title="Doctor Availability">
        <x-slot name="subtitle">Manage doctor schedules and availability</x-slot>
        <x-slot name="actions">
                ...
                <tbody class="divide-y divide-surface-border">
                    @forelse($availabilities as $avail)
                        <tr class="hover:bg-surface transition-colors">
                            <td class="px-4 py-3">
                                <div class="flex items-center gap-3">
                                    <div class="w-10 h-10 rounded-full bg-primary-500/20 flex items-center justify-center overflow-hidden flex-shrink-0">
                                        @if($avail->doctor->photo)
                                            <img src="{{ asset('uploads/doctors/' . $avail->doctor->photo) }}" alt="{{ $avail->doctor->name }}" class="w-full h-full object-cover">
                                        @else
                                            <span class="text-sm font-medium text-primary-600">{{ strtoupper(substr($avail->doctor->name, 0, 2)) }}</span>
                                        @endif
                                    </div>
                                    <div>
                                        <p class="text-sm font-medium text-text-primary">{{ $avail->doctor->name }}</p>
                                        <p class="text-xs text-text-muted">{{ $avail->doctor->email }}</p>
                                    </div>
                                </div>
                            </td>
                            <td class="px-4 py-3 text-sm text-text-primary capitalize">{{ $avail->day }}</td>
                            ...
                            <td class="px-4 py-3">
                                <x-status-badge :variant="$avail->status" :label="$avail->status_label" />
                            </td>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                <x-empty-state message="No availability schedules found." />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
<x-modal id="create-availability" title="Add Availability">
    <form id="create-avail-form" class="space-y-4">
        @csrf
        <div>
            <label class="block text-sm font-medium text-text-primary mb-1.5">Doctor <span class="text-red-500">*</span></label>
            <select name="doctor_id" id="create-avail-doctor" class="w-full px-3 py-2 text-sm bg-white border border-surface-border rounded-lg focus:outline-none focus:ring-2 focus:ring-primary-500/20 focus:border-primary-500" required>
                <option value="">Select Doctor</option>
                @foreach(\App\Models\Doctor::where('status', 'active')->get() as $doctor)
...
<x-modal id="edit-availability" title="Edit Availability">
...
async function submitCreateAvailability() {
    ...
    try {
        ...
        const response = await fetch('{{ route("admin.availability.store") }}', {
            method: 'POST',
            body: formData,
            headers: {
...
async function editAvailability(id) {
    try {
        const response = await fetch(`/admin/availability/${id}`, {
...
async function toggleAvailability(id) {
    try {
        const response = await fetch(`/admin/availability/${id}/toggle`, {
            method: 'PUT',
            headers: {
                'X-CSRF-TOKEN': document.querySelector('meta[name="csrf-token"]').content,
                'X-Requested-With': 'XMLHttpRequest',
...
async function deleteAvailability(id) {
    confirmAction('Are you sure you want to delete this availability?', async () => {
        try {
            const response = await fetch(`/admin/availability/${id}`, {
                method: 'DELETE',
                headers: {
