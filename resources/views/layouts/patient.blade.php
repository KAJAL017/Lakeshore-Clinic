...
<head>
    ...
    <style>
        .sidebar { background-color: #134e4a; }
        .sidebar-link { transition: all 0.2s ease; color: rgba(255,255,255,0.6); }
        .sidebar-link:hover { background: rgba(255,255,255,0.08); color: #fff; }
        .sidebar-link.active { background: rgba(255,255,255,0.12); color: #fff; box-shadow: inset 3px 0 0 0 #5eead4; }
        ...
        .sidebar-user-sub { color: rgba(94,234,212,0.5); }
...
<body class="h-full bg-page">
    ...
    <div id="confirmation-dialog" class="hidden fixed inset-0 z-50 flex items-center justify-center bg-black/40" style="backdrop-filter: blur(4px);">
        <div class="bg-white rounded-2xl shadow-2xl p-6 max-w-sm w-full mx-4">
            <div class="flex items-center gap-3 mb-4">
                ...
                <div><h3 class="text-lg font-semibold text-gray-900">Confirm Action</h3></div>
            </div>
            <p class="confirm-message text-gray-500 text-sm mb-6"></p>
            <div class="flex justify-end gap-3">
                <button class="cancel-btn px-4 py-2 text-sm font-medium text-gray-600 bg-gray-100 rounded-xl hover:bg-gray-200 transition-colors">Cancel</button>
                <button class="confirm-btn px-4 py-2 text-sm font-medium text-white bg-red-500 rounded-xl hover:bg-red-600 transition-colors">Confirm</button>
            </div>
        </div>
    </div>
        ...
        <aside id="sidebar" class="sidebar fixed top-0 left-0 z-40 h-full w-[260px] text-white -translate-x-full lg:translate-x-0 transition-transform duration-300" style="transition-property: transform;">
            ...
            <nav class="flex-1 overflow-y-auto py-4 px-3 scrollbar-hide">
                ...
                <ul class="space-y-0.5">
                    <li><a href="{{ route('patient.dashboard') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.dashboard') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 12l2-2m0 0l7-7 7 7M5 10v10a1 1 0 001 1h3m10-11l2 2m-2-2v10a1 1 0 01-1 1h-3m-6 0a1 1 0 001-1v-4a1 1 0 011-1h2a1 1 0 011 1v4a1 1 0 001 1m-6 0h6"/></svg><span>Dashboard</span></a></li>
                    <li><a href="{{ route('patient.book-appointment') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.book-appointment*') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M12 4v16m8-8H4"/></svg><span>Book Appointment</span></a></li>
                    <li><a href="{{ route('patient.my-appointments') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.my-appointments') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg><span>My Appointments</span></a></li>
                </ul>

                <p class="sidebar-section-label">Health</p>
                <ul class="space-y-0.5">
                    <li><a href="{{ route('patient.medical-records') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.medical-records*') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg><span>Medical Records</span></a></li>
                    <li><a href="{{ route('patient.telemedicine-sessions') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.telemedicine-sessions') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 10l4.553-2.276A1 1 0 0121 8.618v6.764a1 1 0 01-1.447.894L15 14M5 18h8a2 2 0 002-2V8a2 2 0 00-2-2H5a2 2 0 00-2 2v8a2 2 0 002 2z"/></svg><span>Telemedicine</span></a></li>
                </ul>

                <p class="sidebar-section-label">Finance</p>
                <ul class="space-y-0.5">
                    <li><a href="{{ route('patient.payments') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.payments*') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg><span>Payments</span></a></li>
                    <li><a href="{{ route('patient.insurance') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.insurance*') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 12l2 2 4-4m5.618-4.016A11.955 11.955 0 0112 2.944a11.955 11.955 0 01-8.618 3.04A12.02 12.02 0 003 9c0 5.591 3.824 10.29 9 11.622 5.176-1.332 9-6.03 9-11.622 0-1.042-.133-2.052-.382-3.016z"/></svg><span>Insurance</span></a></li>
                </ul>

                <p class="sidebar-section-label">Account</p>
                <ul class="space-y-0.5">
                    <li><a href="{{ route('notifications') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('notifications') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M15 17h5l-1.405-1.405A2.032 2.032 0 0118 14.158V11a6.002 6.002 0 00-4-5.659V5a2 2 0 10-4 0v.341C7.67 6.165 6 8.388 6 11v3.159c0 .538-.214 1.055-.595 1.436L4 17h5m6 0v1a3 3 0 11-6 0v-1m6 0H9"/></svg><span>Notifications</span></a></li>
                    <li><a href="{{ route('patient.profile') }}" class="sidebar-link flex items-center gap-3 px-3 py-2.5 rounded-xl text-sm font-medium {{ request()->routeIs('patient.profile*') ? 'active' : '' }}"><svg class="w-5 h-5 flex-shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg><span>My Profile</span></a></li>
                </ul>
            </nav>
            ...
            <div class="p-3" style="border-top: 1px solid rgba(255,255,255,0.05);">
                ...
                <div class="flex items-center gap-3 px-3 py-2.5 rounded-xl sidebar-user-bg">
                    <div class="w-9 h-9 rounded-full flex-shrink-0 flex items-center justify-center overflow-hidden" style="background: linear-gradient(135deg, #14b8a6, #0d9488);">
                        @php $photo = auth()->user()->patient?->photo ?? null; @endphp
                        @if($photo)
                            <img src="{{ asset('uploads/patients/' . $photo) }}" alt="" class="w-full h-full object-cover">
                        @else
                            <span class="text-xs font-bold text-white">{{ strtoupper(substr(auth()->user()->name ?? 'P', 0, 2)) }}</span>
                        @endif
                    </div>
                    <div class="flex-1 min-w-0 hidden xl:block">
                        <p class="text-sm font-medium text-white truncate">{{ auth()->user()->name ?? 'Patient' }}</p>
                        <p class="text-xs sidebar-user-sub truncate">Patient</p>
                    </div>
                </div>
                @endauth
            </div>
                    ...
                    <div class="flex items-center gap-2">
                        <div class="relative">
                            <button id="profile-dropdown-btn" class="flex items-center gap-3 p-1.5 rounded-xl hover:bg-gray-100 transition-colors">
                                <div class="w-8 h-8 rounded-full flex items-center justify-center overflow-hidden" style="background: linear-gradient(135deg, #14b8a6, #0d9488);">
                                    <span class="text-xs font-bold text-white">{{ strtoupper(substr(auth()->user()->name ?? 'P', 0, 2)) }}</span>
                                </div>
                            ...
                            <div id="profile-dropdown" class="hidden absolute right-0 top-12 mt-2 w-56 bg-white rounded-2xl shadow-xl border border-gray-100 overflow-hidden z-50">
                                <div class="px-4 py-3" style="border-bottom: 1px solid #f3f4f6;">
                                    <p class="text-sm font-semibold text-gray-900">{{ auth()->user()->name ?? 'Patient' }}</p>
                                    <p class="text-xs text-gray-400">{{ auth()->user()->email ?? '' }}</p>
                                </div>
                                <div class="py-1">
                                    <a href="{{ route('patient.profile') }}" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-gray-600 hover:bg-gray-50 transition-colors"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"/></svg>My Profile</a>
                                </div>
                                <div class="py-1" style="border-top: 1px solid #f3f4f6;">
                                    <button id="logout-btn" class="flex items-center gap-2.5 px-4 py-2.5 text-sm text-red-500 hover:bg-red-50 transition-colors w-full"><svg class="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 16l4-4m0 0l-4-4m4 4H7m6 4v1a3 3 0 01-3 3H6a3 3 0 01-3-3V7a3 3 0 013-3h4a3 3 0 013 3v1"/></svg>Logout</button>
                                </div>
                            </div>
                        </div>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        if (profileBtn && profileDropdown) {
            ...
            document.addEventListener('click', function() { profileDropdown.classList.add('hidden'); });
        }
