...
<div class="space-y-6">
    <x-page-header title="My Appointments">
        <x-slot name="subtitle">View your appointment history</x-slot>
        <x-slot name="actions">
                        ...
                        <tr>
                            <td colspan="5" class="px-4 py-12 text-center">
                                <x-empty-state message="No appointments found.">
                                    <x-button variant="primary" size="sm" href="{{ route('patient.booking') }}">Book an Appointment</x-button>
                                </x-empty-state>
                            </td>
                        </tr>
                    @endforelse
            ...
            const content = \`
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    <div>
                        ...
                        <p class="text-xs text-text-muted text-xs">\${a.appointment_time}</p>
                    </div>
                    <div>
                        <p class="text-text-muted mb-1">Reason</p>
                        <p class="text-text-primary">\${a.reason || 'Not specified'}</p>
                    </div>
                    \${a.symptoms ? \`<div class="col-span-2"><p class="text-text-muted mb-1">Symptoms</p><p class="text-text-primary">\${a.symptoms}</p></div>\` : ''}
                    \${a.notes ? \`<div class="col-span-2"><p class="text-text-muted mb-1">Notes</p><p class="text-text-primary">\${a.notes}</p></div>\` : ''}
                </div>
            \`;
