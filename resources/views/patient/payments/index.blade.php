...
@section('title', 'My Payments - Lakeshore Clinic')
@section('page-title', 'Payment History')

@section('content')
<div class="space-y-6">
    <x-page-header title="Payment History">
        <x-slot name="subtitle">View your payment transactions</x-slot>
    </x-page-header>
                    ...
                    @empty
                        <tr>
                            <td colspan="6" class="px-4 py-12 text-center">
                                <x-empty-state message="No payment history found." />
                            </td>
                        </tr>
                    @endforelse
                </tbody>
...
async function viewPayment(id) {
    try {
        const response = await fetch(`/patient/payments/${id}`, {
        ...
        if (data.success) {
            ...
            document.getElementById('payment-content').innerHTML = `
                <div class="grid grid-cols-2 gap-4 text-sm">
                    ...
                    ${p.appointment ? `<div class="col-span-2"><p class="text-text-muted mb-1">Appointment</p><p class="text-text-primary">${p.appointment.type_label} - ${new Date(p.appointment.appointment_date).toLocaleDateString()}</p></div>` : ''}
                </div>
            `;
